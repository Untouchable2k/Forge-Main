  ...
  contract ForgeAuctions is  GasPump, IERC20, Ownabled
{
    ...
    event BurnMultipleDays(address indexed payer, address indexed member, uint era, uint NumberOfDays, uint totalUnits, uint dailyTotal);
...
*/
    ...
    for(uint256 x = 0; x < fdays.length; x++)
    {
        ...
        emit BurnMultipleDays(msg.sender, _member, _era, fdays.length, stricttotal, mapEraDay_Units[_era][_day]);
        return true;
    }
    function burn0xBTCForMember(address member, uint256 _0xbtcAmount) public  {
        require(IERC20(ZeroXBTCAddress).transferFrom(msg.sender, burnAddress, amt), "NO WAY, requires 0xBTC send");


        _recordBurn(msg.sender, member, currentEra, currentDay, _0xbtcAmount);
        emit Burn(_payer, _member, _era, _day, _0xbtcAmount, mapEraDay_Units[_era][_day]);
    ...
    function _recordBurn(address _payer, address _member, uint _era, uint _day, uint _eth, bool multidays) private {
    ...
    function WithdrawEz(address _member) public
    {
        ...
        uint startDay
        for(int y=startingera; y <= currentera; y++){
          if(y != currentEra)
           {
              startingday = daysPerEra;
            }
           uint[] memory dd = new uint[](currentDay-startingday); 
           for(uint x=startinday; x< currentDay; x++)
           {
               dd[x-startinday] = x ;
           }
