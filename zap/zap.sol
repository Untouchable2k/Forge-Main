...
contract SwapRouter {
    function swapExactTokensForTokens(uint256 amountIn, uint256 amountOutMin, address[] calldata path, address to, uint256 deadline) public returns (bool){}
    function swapExactETHForTokens(uint256 amountOutMin, address[] calldata path, address to, uint256 deadline) public payable returns (bool){}
    }
  ...
  contract ForgeZap is  GasPump, OwnableAndMods
{
    ...
    // Constructor
      ...
      constructor()  {
        ...
        AddressLP3 = address(0xFe2419430C29be0EC000317E53BaB14836543ec2);
...
function perfect (uint256 amt) public payable returns (bool success){
    ...
    //Stake
    //Return Leftovers
    ZeroxBTCToForge(2, 11 );
    IERC20(AddressZeroXBTC).transferFrom(msg.sender, address(this), amt);
    amt = IERC20(AddressForgeToken).balanceOf(address(this));
    uint256 amt2 = (amt / 2 * 1003)/1000;
    ZeroxBTCToForge(1, amt2);
    LPandStake();
...
function LPandStake() public returns (bool success){
        require(IERC20(AddressLP3).approve(AddressStake, 100 * 10 ** 18), 'approve failed.');
        ...
        //call LP
        LP3.addLiquidity(AddressForgeToken, AddressZeroXBTC, total0xBTCin, totalForgein,  (total0xBTCin*95) /100, totalForgein * 95 /100, address(this), block.timestamp + 1000);
        
        Forge_Staking.stakeFor(msg.sender, uint128(IERC20(LPPool0xBTCForge).balanceOf(address(this))));
    return true;
    }
...
function ZeroxBTCToForge(uint choice, uint256 amountIn) public payable returns (bool success) {
    ...
    address[] memory path = new address[](2);
        ...
        path[0] = AddressMatic;
        path[1] = AddressUSDC;
        path[2] = AddressZeroXBTC;
        PolygonTOZeroX(1, msg.sender, path);
        }else if(choice == 1){
...
function PolygonTOZeroX(uint256 haircut, address whoToStakeFor, address[] memory path) public payable returns (bool success) {
...
function ForgeToZeroXBTC(uint256 amountIn0xBTC, uint256 haircut, address whoToStakeFor, address[] memory path) public payable returns (bool success) {
...
function ZeroxBTCToForge2(uint256 amountInForge, uint256 haircut, address whoToStakeFor, address[] memory path) public payable returns (bool success) {
        uint256 startBal0xBTC = IERC20(AddressZeroXBTC).balanceOf(address(this));
	//haircut is what % we will loose in a trade if someone frontruns set at 97% in contract now change for public
        require(IERC20(AddressZeroXBTC).approve(address(Quickswap), 100 * 10 ** 18), 'approve failed.');
...
function OneTeste1(uint256 amountOutMin) public  {
            ...
            sixsevenths = (expectedProfit * 6) / 7;
            }
